"""
Gobierno Querétaro - Water CEA Agent FastAPI Server
"""
...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"Starting Water CEA Agent: {settings.agent_id}")
    agent = get_agent()
...
app = FastAPI(
    title="Gobierno Querétaro - Water CEA Agent",
    description="LangGraph-based water services agent for CEA Querétaro",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def query_agent(request: QueryRequest):
    ...
    try:
        ...
        folio_match = re.search(r"CEA-\d{8}-[A-Z0-9]{4}", response_text)
        if folio_match:
...
async def agent_info():
    ...
    return {
        ...
        "requires_contract": agent.config.get("requires_contract", True),
    }
...
async def root():
    """Root endpoint"""
    return {
        "service": "Gobierno Querétaro - Water CEA Agent",
        "agent_id": settings.agent_id,
        "status": "running",
        "endpoints": {
