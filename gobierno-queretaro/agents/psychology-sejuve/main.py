"""
Gobierno Queretaro - Psychology SEJUVE Agent FastAPI Server
"""
...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"Starting Psychology SEJUVE Agent: {settings.agent_id}")
    agent = get_agent()
...
app = FastAPI(
    title="Gobierno Queretaro - Psychology SEJUVE Agent",
    description="LangGraph-based psychology/mental health services agent",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def query_agent(request: QueryRequest):
    """Query the Psychology SEJUVE agent"""
    try:
        agent = get_agent()
        ...
        is_crisis = result.get("subtask_results", {}).get("crisis", {}).get("is_crisis", False)

        # Check for ticket folio
        ticket_folio = None
        response_text = result.get("response", "")
        folio_match = re.search(r"PSI-\d{8}-[A-Z0-9]{4}", response_text)
        if folio_match:
...
async def agent_info():
    ...
    return {
        ...
        "category_code": agent.config["category_code"],
        "task_types": agent.config["task_types"],
        "requires_contract": agent.config.get("requires_contract", False),
        "crisis_line": "800-911-2000",
    }
...
async def root():
    """Root endpoint"""
    return {
        "service": "Gobierno Queretaro - Psychology SEJUVE Agent",
        "agent_id": settings.agent_id,
        "status": "running",
        "crisis_line": "800-911-2000",
        "endpoints": {
