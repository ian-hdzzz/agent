"""Gobierno Queretaro - Social SEDESOQ Agent FastAPI Server"""
import os as _os
...
async def lifespan(app: FastAPI):
    logger.info(f"Starting Social SEDESOQ Agent: {settings.agent_id}")
    agent = get_agent()
    logger.info(f"Agent initialized: {agent.config['name']}")
    yield
    logger.info(f"Shutting down: {settings.agent_id}")

app = FastAPI(title="Gobierno Queretaro - Social SEDESOQ Agent", version="1.0.0", lifespan=lifespan)
...
async def query_agent(request: QueryRequest):
    try:
        ...
        folio_match = re.search(r"SOC-\d{8}-[A-Z0-9]{4}", response_text)
...
@app.get("/")
async def root():
    return {"service": "Gobierno Queretaro - Social SEDESOQ Agent", "agent_id": settings.agent_id, "status": "running", "endpoints": {"agent_card": "/.well-known/agent.json", "playground": "/playground/ui"}}
