"""
Gobierno Queretaro - Transport AMEQ Agent FastAPI Server
"""
...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"Starting Transport AMEQ Agent: {settings.agent_id}")
    agent = get_agent()
...
app = FastAPI(
    title="Gobierno Queretaro - Transport AMEQ Agent",
    description="LangGraph-based transport services agent for AMEQ Queretaro",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def query_agent(request: QueryRequest):
    """Query the Transport AMEQ agent"""
    try:
        agent = get_agent()
        ...
        folio_match = re.search(r"TRA-\d{8}-[A-Z0-9]{4}", response_text)
        if folio_match:
...
async def root():
    """Root endpoint"""
    return {
        "service": "Gobierno Queretaro - Transport AMEQ Agent",
        "agent_id": settings.agent_id,
        "status": "running",
        "endpoints": {
