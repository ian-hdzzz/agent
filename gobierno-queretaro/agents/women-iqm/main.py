"""
Gobierno Queretaro - Women IQM Agent FastAPI Server
"""
...
async def lifespan(app: FastAPI):
    logger.info(f"Starting Women IQM Agent: {settings.agent_id}")
    agent = get_agent()
...
app = FastAPI(title="Gobierno Queretaro - Women IQM Agent", description="Women's services agent", version="1.0.0", lifespan=lifespan)
...
async def query_agent(request: QueryRequest):
    try:
        ...
        is_emergency = result.get("subtask_results", {}).get("violence", {}).get("is_emergency", False)

        ticket_folio = None
        response_text = result.get("response", "")
        folio_match = re.search(r"IQM-\d{8}-[A-Z0-9]{4}", response_text)
        if folio_match:
            ticket_folio = folio_match.group()

        return QueryResponse(response=result.get("response", ""), agent_id=result.get("agent_id", settings.agent_id),
                           conversation_id=request.conversation_id, task_type=result.get("task_type"),
                           tools_used=tools_used, ticket_folio=ticket_folio, is_emergency=is_emergency, error=result.get("error"))
...
async def agent_info():
    agent = get_agent()
    return {"agent_id": agent.config["id"], "agent_name": agent.config["name"], "category_code": agent.config["category_code"],
            "task_types": agent.config["task_types"], "emergency_line": "800-670-3737"}
...
@app.get("/")
async def root():
    return {"service": "Gobierno Queretaro - Women IQM Agent", "agent_id": settings.agent_id, "status": "running", "emergency_line": "800-670-3737", "endpoints": {"agent_card": "/.well-known/agent.json", "playground": "/playground/ui"}}
