"""
Gobierno Queretaro - Education USEBEQ Agent FastAPI Server
"""
...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"Starting Education USEBEQ Agent: {settings.agent_id}")
    agent = get_agent()
...
app = FastAPI(
    title="Gobierno Queretaro - Education USEBEQ Agent",
    description="LangGraph-based education services agent for USEBEQ Queretaro",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def query_agent(request: QueryRequest):
    """Query the Education USEBEQ agent"""
    try:
        agent = get_agent()
        ...
        folio_match = re.search(r"EDU-\d{8}-[A-Z0-9]{4}", response_text)
        if folio_match:
...
async def root():
    """Root endpoint"""
    return {
        "service": "Gobierno Queretaro - Education USEBEQ Agent",
        "agent_id": settings.agent_id,
        "status": "running",
        "endpoints": {
