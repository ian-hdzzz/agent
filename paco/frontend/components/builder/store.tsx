...
// TODO: AutoGen-specific logic cleanup needed:
// - Replace AutoGen provider strings with PACO providers (see line 214)
// - Update component creation to use PACO type system
// - Remove AutoGen-specific workbench/model_client patterns
// These changes are tracked for Phase 2 type system migration.
  ...
  addNode: (
    ...
    set((state) => {
      ...
      if (isTeamComponent(clonedComponent)) {
        ...
        newNodes.push(newNode);
      } else if (isMcpServerComponent(clonedComponent)) {
        // MCP Server node - standalone node that connects to agents
        const newNode: CustomNode = {
          id: nanoid(),
          position,
          type: "mcp-server",
          data: {
            label: clonedComponent.label || clonedComponent.config.name || "MCP Server",
            component: clonedComponent,
            type: "mcp-server" as NodeData["type"],
          },
        };
        newNodes.push(newNode);

        // If dropped on an agent, create edge connection
        if (targetNodeId) {
          const targetNode = state.nodes.find((n) => n.id === targetNodeId);
          if (targetNode && isAgentComponent(targetNode.data.component)) {
            newEdges.push({
              id: nanoid(),
              source: newNode.id,
              target: targetNode.id,
              sourceHandle: `${newNode.id}-mcp-output-handle`,
              targetHandle: `${targetNode.id}-agent-input-handle`,
              type: "mcp-connection",
            });
          }
        }
      } else if (isKnowledgeComponent(clonedComponent)) {
        // Knowledge Base node - standalone node that connects to agents
        const newNode: CustomNode = {
          id: nanoid(),
          position,
          type: "knowledge",
          data: {
            label: clonedComponent.label || clonedComponent.config.name || "Knowledge Base",
            component: clonedComponent,
            type: "knowledge" as NodeData["type"],
          },
        };
        newNodes.push(newNode);
        ...
        if (targetNodeId) {
          ...
          if (targetNode && isAgentComponent(targetNode.data.component)) {
            newEdges.push({
              ...
              source: newNode.id,
              target: targetNode.id,
              sourceHandle: `${newNode.id}-knowledge-output-handle`,
              targetHandle: `${targetNode.id}-agent-input-handle`,
              type: "knowledge-connection",
            });
          }
        }
      } else if (isLogicComponent(clonedComponent)) {
        // Logic node - standalone node for conditional branching
        const newNode: CustomNode = {
          id: nanoid(),
          position,
          type: "logic",
          data: {
            label: clonedComponent.label || clonedComponent.config.name || "Logic",
            component: clonedComponent,
            type: "logic" as NodeData["type"],
          },
        };
        newNodes.push(newNode);
